...
|-----------------------------------|------------|--------|--------------------------|
| Natural mortality (M)             | 5          | 5      | 0.2, 0.3, 0.4, 0.4-0.3, 0.4-0.2 ^[First figure refers to juveniles, second one to adults] |
...
# Annex A - Modifications to SS control and data files

The following sections show how the manipulation of the original Stock Synthesis (3.30) input files has been carried out. In the code below `value` refers to a vector of length one or larger, `ctl` to the SS control file, and `dat` to the SS data file. Files are loaded and recreated using the corresponding functions in the *r4ss* package.

## Natural mortality (M)

```{r, eval=FALSE, echo=TRUE}
ctl$MG_parms[c("NatM_p_1_Fem_GP_1", "NatM_p_1_Mal_GP_1"), "INIT"] <- value
```

## Standard deviation of recruitment deviances (sigmaR)

```{r, eval=FALSE, echo=TRUE}
ctl$SR_parms["SR_sigmaR", "INIT"] <- value
```

## Steepness of the Beverton-Holt stock-recruits relationship.

```{r, eval=FALSE, echo=TRUE}
ctl$SR_parms["SR_BH_steep", "INIT"] <- value
```
